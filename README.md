...
登录管理员账号，在首页打开“销售统计”，可查询本周、本月、近 7/30 天或最多 366 天的
自选日期。提供出库、销售撤销、净出库的每日、平台和游戏汇总。结束日期包含全天，
时区使用 `APP_TIMEZONE`；撤销计入实际撤销当天，不回写原销售日。统计件数，不统计金额。
历史名称/平台变化会分别展示。更多口径和后续范围见 [实施记录](docs/implementation-status.md)。


### 库存筛选与排序

管理员打开“库存与预警管理”，可搜索名称、目录别名、条码、平台、地区、版本。
展开“筛选和排序”，可组合精确的平台/地区/版本、库存状态和期间销量条件，选择升降序。
零销量与低销量清单只包含当前有库存的启用商品；低销量默认 1～3 件，上限可调整。
期间支持本周、本月、近 7/30 天、自选日期（最多 366 天），与销售报表使用同一业务时区。
期间入库只计原始 IN，期间销量/出库只计原始 SALE_OUT，均不扣除撤销、不含盘点调整。
