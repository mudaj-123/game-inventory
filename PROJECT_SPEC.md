...
> **历史规格提示（2026-09-20）：** 本文件包含项目最初以云服务器为主要部署目标时的
> 完整需求背景，保留用于追溯。当前权威需求已经更新为
> [`docs/CURRENT_REQUIREMENTS.md`](docs/CURRENT_REQUIREMENTS.md)：主要生产环境是店铺内的
> Windows 10 电脑，原生运行 PostgreSQL 与 FastAPI；Docker/云部署仅作为可选方案。
> 实施工作遇到冲突时必须遵循当前需求文档，并优先保护已有数据和库存事务语义。
